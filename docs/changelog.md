...
本文档记录系统的所有重要改动，包含问题分析、解决方案与关键代码片段。
...
### 核心功能
- [文件上传入库（4步向导）](#13-文件上传入库4步向导) `2026-03-17`
- [上传配置 AI 助手（多轮对话）](#17-上传配置-ai-助手多轮对话) `2026-03-18`
- [AI 分块分析流式输出](#14-ai-分块分析流式输出) `2026-03-17`
- [飞书 WebSocket 长连接机器人](#9-飞书-websocket-长连接机器人)
- [回答质量评价体系](#10-回答质量评价体系)
- [MCP 数据源集成](#11-mcp-数据源集成)
- [检索增强 Advanced RAG](#121-检索增强-advanced-rag)

### 数据处理优化
- [内容 Hash 去重（跨文件重叠处理）](#16-内容-hash-去重跨文件重叠处理) `2026-03-17`
- [Milvus gRPC 64MB 批量写入修复](#15-milvus-grpc-64mb-批量写入修复) `2026-03-17`
- [Milvus VARCHAR 字节溢出修复](#1-milvus-varchar-字节溢出修复)

### 查询路由与检索
- [元数据精确过滤路由](#6-元数据精确过滤路由)
- [聚合查询修复](#5-聚合查询修复)
- [聚合关键词量词变体正则修复](#7-聚合关键词量词变体正则修复)

### API 与集成
- [Chat API 流式响应兼容](#2-chat-api-流式响应兼容)
- [OpenAPI 规范化（OpenClaw 接入）](#4-openapi-规范化openclaw-接入)

### 基础设施
- [日志系统增强](#3-日志系统增强)
- [开源前脱敏处理](#8-开源前脱敏处理)
- [hashlib 缺失导入修复](#18-hashlib-缺失导入修复) `2026-03-18`
...
BM25 当前使用**按单字拆分**的中文分词（`"客户拜访"` → `["客","户","拜","访"]`），关键词精确匹配效果有限。计划引入 `jieba` 做词级分词，详见 TODO.md。
